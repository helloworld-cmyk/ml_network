"""OCR ảnh trang PDF (từ folder _preview_pages1) qua servernvidia proxy (OpenAI-compatible).

Logic trang/câu giống core/script.py; gọi proxy local thay vì Ollama trực tiếp.

Usage:
  # Chạy proxy trước: cd servernvidia && python main.py  → http://127.0.0.1:1235
  python core/proxy.py
  python core/proxy.py -o .md/output.txt
  python core/proxy.py --start-page 8
  python core/proxy.py --model minimax-m3
  python core/proxy.py --images-dir rawtext/_preview_pages1 --base-url http://127.0.0.1:1235/v1
"""
...
# servernvidia .env hiện dùng PORT=1235 (không phải 1234 mặc định README).
DEFAULT_BASE_URL = os.environ.get(
	"PROXY_BASE_URL",
	os.environ.get("OPENAI_BASE_URL", "http://127.0.0.1:1235/v1"),
)
DEFAULT_API_KEY = os.environ.get("PROXY_API_KEY", "local")
...
def _chat_completions_url(base_url: str) -> str:
	return urljoin(_normalize_base_url(base_url), "chat/completions")


def _models_url(base_url: str) -> str:
	return urljoin(_normalize_base_url(base_url), "models")
...
def fetch_proxy_models(base_url: str, api_key: str) -> list[dict[str, Any]]:
	"""Lấy danh sách model từ GET /v1/models của servernvidia proxy."""
	try:
		...
		raise RuntimeError(
			f"Không kết nối được proxy tại {base_url}. "
			f"Chạy `python main.py` trong servernvidia rồi thử lại. Chi tiết: {exc}"
		) from exc

	if not response.ok:
		raise RuntimeError(
			f"Proxy /v1/models lỗi {response.status_code}: {response.text.strip()}"
		)

	payload = response.json()
	data = payload.get("data") if isinstance(payload, dict) else None
	if not isinstance(data, list):
		raise RuntimeError(f"Phản hồi /v1/models không hợp lệ: {payload!r}")
...
def ask_model(
	...
	"""Chọn model OCR: --model nếu có, không thì hỏi tương tác từ /v1/models."""
	available = fetch_proxy_models(base_url, api_key)
	ids = [item["id"] for item in available]
	vision_ids = [item["id"] for item in available if item.get("supports_vision")]

	suggested = DEFAULT_MODEL
	if suggested not in ids and vision_ids:
		suggested = vision_ids[0]
	elif suggested not in ids and ids:
		suggested = ids[0]

	if cli_model is not None:
		if ids and cli_model not in ids:
			print(
				f"Cảnh báo: '{cli_model}' không có trong /v1/models "
	...
	if not available:
		print(
			f"Không thấy model nào từ proxy. Dùng mặc định: {suggested}",
			flush=True,
		)
		return suggested

	print(f"\nModels từ proxy ({base_url}):", flush=True)
	for index, item in enumerate(available, start=1):
		flags: list[str] = []
		if item.get("supports_vision"):
			flags.append("vision/OCR")
		if item.get("provider"):
			flags.append(str(item["provider"]))
		if item.get("role"):
			flags.append(str(item["role"]))
		suffix = f"  [{', '.join(flags)}]" if flags else ""
		marker = " ← mặc định OCR" if item["id"] == suggested else ""
		print(f"  {index:2d}. {item['id']}{suffix}{marker}", flush=True)

	while True:
		raw = input(
			f"Chọn model OCR [1-{len(available)}] hoặc nhập id "
			f"(Enter = {suggested}): "
		).strip()
		if not raw:
			return suggested
...
def ensure_proxy_ready(base_url: str, api_key: str, model: str) -> None:
	"""Kiểm tra proxy sống và model resolve được."""
	print(f"Đang kết nối proxy {base_url} (model '{model}')...", flush=True)
	started = time.monotonic()
	available = fetch_proxy_models(base_url, api_key)
	ids = [item["id"] for item in available]
	if ids:
		print(f"  Models có sẵn: {', '.join(ids)}", flush=True)
		if model not in ids:
			print(
				f"  Cảnh báo: '{model}' không nằm trong danh sách /v1/models.",
				file=sys.stderr,
			)
	else:
		print("  Cảnh báo: /v1/models trống.", file=sys.stderr)
	print(f"Đã sẵn sàng proxy ({time.monotonic() - started:.1f}s).", flush=True)


def _extract_stream_error(chunk: dict[str, Any]) -> str | None:
	"""Lấy message lỗi từ SSE chunk kiểu OpenAI/proxy (HTTP 200 nhưng body có error)."""
	err = chunk.get("error")
	if err is None:
		return None
...
def _raise_for_proxy_error(response: requests.Response) -> None:
	if response.ok:
		return
	...
	try:
		...
		pass
	raise RuntimeError(f"Proxy API lỗi {response.status_code}: {detail}")
...
def ocr_image(
	...
	*,
	...
	try:
		...
		_raise_for_proxy_error(response)
		...
		for raw_line in response.iter_lines(decode_unicode=True):
			...
			# servernvidia stream: lỗi upstream (vd 429) vẫn gửi HTTP 200 + SSE error
			stream_error = _extract_stream_error(chunk)
			if stream_error:
				raise RuntimeError(f"Proxy stream lỗi: {stream_error}")
	...
	if not text:
		raise RuntimeError(
			f"{prefix} OCR trả về rỗng (có thể rate-limit / stream lỗi bị nuốt). "
			"Không ghi trang trống — thử lại sau vài giây."
		)
...
def images_to_text(
	...
	ensure_proxy_ready(base_url, api_key, model)

	output_path.parent.mkdir(parents=True, exist_ok=True)
	print(f"Proxy: {base_url}", flush=True)
...
def main() -> int:
	parser = argparse.ArgumentParser(
		description=(
			"OCR ảnh trang qua servernvidia proxy (OpenAI-compatible). "
			"Chạy `python main.py` trong servernvidia trước."
		)
	)
	parser.add_argument(
		"--images-dir",
	...
	parser.add_argument(
		...
		default=None,
		help=(
			"Alias model trên proxy (vd: minimax-m3, minimax-m3-local). "
			"Bỏ trống để chọn tương tác từ GET /v1/models."
		),
	)
	parser.add_argument(
		"--base-url",
		default=DEFAULT_BASE_URL,
		help=f"URL OpenAI-compatible của proxy (mặc định {DEFAULT_BASE_URL})",
	)
	parser.add_argument(
		"--api-key",
	...
	try:
		...
		model = ask_model(args.base_url, args.api_key, args.model)
		print(f"Đã chọn model: {model}", flush=True)
		images_to_text(
			images_dir,
			output_path,
			model,
